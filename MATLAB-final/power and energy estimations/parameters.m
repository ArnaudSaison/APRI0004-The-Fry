...
par.takeoff_blown_wing_effect = 2;              % coefficient from blown lift effet
...
%--------------------------------------------------------------------------
par.prop_efficiency_to = 0.5;           % at take-off
par.prop_efficiency_cruise = 0.8;       % 
...
% Cruise
%--------------------------------------------------------------------------
par.cruise_altitude = 14000 * 0.3048;       % [m]
...
par.climb_blown_wing_effect = 1.0;          % []
%par.C_L_climb = par.C_L_max;
par.C_L_climb = 1.3;                        % [] based on climb_optimizer.m
par.C_D0_climb = par.C_D0_min * 1.2;        % []

%--------------------------------------------------------------------------
% Landing 
%--------------------------------------------------------------------------
par.mean_deceleration = 0.28; % tabulated value for light aircraft with breaks
...
par.nb_HLP_engines = 8;                      % number of high lift propeller engines
par.mass_power_elec = par.nb_HLP_engines * 10 + 2 * 35;% [kg] 8 * 10
par.mass_power_ICE = 150;                    % 

par.peak_power_elec = 1.3;                   % [-] coefficient multiplying electric engine to have peak power

par.generator_fuel_efficiency = 0.50;        % [-] from primary energy to electric energy at cruise altitude
par.charging_efficiency = 0.98;              % [-] charging efficiency between ICE and battery
par.electric_engines_efficiency = 0.98;      % [-]
