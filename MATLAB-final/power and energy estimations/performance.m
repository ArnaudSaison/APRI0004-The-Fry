...
ground_distance = x_g == V_LOF^2 / (2 * par.h) / (T_mean / par.W_to - mu_p);
...
% theoretical C_L
%par.C_L_minD = 2 * par.W_to / (par.rho_cruise * res.cruise.V^2 * par.S);
...
res.landing.V_TD = 1 * sqrt(2 * par.W_to / (par.rho_0 * par.S * par.C_L_max));
res.landing.ground_distance = res.landing.V_TD^2 / 2 * par.mean_deceleration;
