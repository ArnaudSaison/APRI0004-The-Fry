W_dg=3637.627;% gross weight lb
W_ev=zeros(1,10);
for i=1:10
%% important parameters
S_w_feet=15*10.7639;
W_fw=0; %fuel weight in wing lb
Tot_W=0;
%% Wing parameters
A=8; %aspect Ratio
...
c_tip=c_root/lambda;
density_aircruise=0.04744525;% lb/ft^3
aircraft_speed=288.714;%feet/sec
q=0.5*density_aircruise*aircraft_speed^2;%dynamic pressure lb/ft^2
max_t=0.039370079; % maximum thickness [feet]
t_over_c=max_t/c_mean;% thickness to cord ratio
n=3.9;%load factor (we chose the maximum via wikipedia)
N_z=1.5*n; %ultimate load factor

%% wing equations
W_wing=0.036*S_w_feet^(0.758)*W_fw^(0.0035)*(A/(cos(Lambda)^2))^(0.6)*q^(0.006)*lambda^(0.04)*(100*t_over_c/(cos(Lambda)))^(-0.3)*(N_z*W_dg)^(0.49);
Tot_W=Tot_W+W_wing;
...
W_horizontal_tail=0.016*(N_z*W_dg)^(0.414)*q^(0.168)*S_ht^(0.896)*(100*t_over_c_ht/cos(Lambda_htail))^(-0.12)*(A/(cos(Lambda_htail)^2))^(0.043)*lambda_h^(-0.02);
...
W_vertical_tail=0.073*(1+0.2*ht_over_hv)*(N_z*W_dg)^(0.376)*q^(0.122)*S_vt^(0.873)*(100*t_over_c_vt/cos(Lambda_vt))^(-0.49)*(A/(cos(Lambda_vt)^(2)))^(0.357)*lambda_vt^(0.039);
...
Tot_W=Tot_W+0.04*W_dg; %4% of total weight for landing gear)

%% installed engine parameters
W_en=56.36;%engine weight, each, lb 
N_en=10;%number of engines (total for aircraft)
%% installed engine equation
W_installed_engine=2.575*W_en^(0.922)*N_en;
...
W_flight_control=0.053*L^(1.536)*B_w^(0.371)*(N_z*W_dg*10e-4)^0.8;
...
W_hydraulics=K_h*W_dg^(0.8)*M_design^(0.5);
...
% W_dg=W_dg;
% M=M;
N_p=4.5;%number of personnel onboard (crew and passengers
%% air conditionning and anti-ice equation
W_acaai=0.265*W_dg^(0.52)*N_p^(0.68)*W_avionics^(0.17)*M_design^(0.08);
Tot_W=Tot_W+W_acaai;
