...
cieling=ones(1,length(T))*4.921260000000000e+04;
...
M_cruise=Vcruise/sound_speed(emplacement);
M_C=1.06*M_cruise;
...
hold on
grid on 
plot(VC_f,altitude_ft,'c','LineWidth',lw);
plot(VD_f,altitude_ft,'r','LineWidth',lw);
x=linspace(150,400,length(T));
% plot(x,turbulencelow,'--r','LineWidth',lw)
% plot(x,turbulencehigh,'--r','LineWidth',lw)
plot(x,stratosphere,'--k','LineWidth',lw)
plot(x,design,'--g','LineWidth',lw)
...
t4=text(xpl,cieling(1)-2000,'Lift and thrust limit');
%t1(1).Color='red';
t2(1).Color='green';
t3(1).Color='black';
t4(1).Color='magenta';
t5=text(280,25000,'VC');
t6=text(380,25000,'VD');
t5(1).Color='cyan';
t6(1).Color='red';
t7=text(313,5000,'Constant Mc');
t7(1).Color='Blue';
