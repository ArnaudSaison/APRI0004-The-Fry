...
Param.Simul.TAPER       = 1;    % Taper (0 = Linear or Constant, 1 = Ideal, 2 = Custom)
...
% ------------ Freestream ------------
Param.Air.ALTITUDE      = 3962.4;             % Altitude, [m]
...
% -------------- Blades --------------
Blades.PROFILE_FILE = 'mh117.dat';       % Profile coordinates for 3D view of the final blade
Blades.COEFF_FILE   = 'mh117.mat';  % Airfoil polars for various Reynolds

Blades.OMEGArpm = 2200;     % Rotational speed, [RPM]
Blades.nELEM    = 100;      % Number of blade elements

Blades.nBLADES          = 3;        % Number of blades on the rotor
Blades.RADIUS           = 0.90;      % Radius (from HUB center to tip), [m]
Blades.ROOT_CHORD       = 0.15;     % Root chord (will only be used if linear/constant taper), [m]
Blades.TIP_CHORD        = 0.06;     % Tip chord, [m]
Blades.ROOT_CUTOUT      = 0.15 * Blades.RADIUS;     % Root cutout, [m]
Blades.THETA_TIPdeg     = 10;        % Twist angle at tip, [deg] % 10
Blades.COLL_PITCHdeg    = 17;        % Collective pitch, [deg]
...
Blades.TWIST_MAXdeg = 60;   % Maximal twist (to avoid infinite twist at root), [deg]
