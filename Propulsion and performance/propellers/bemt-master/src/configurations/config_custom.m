...
Param.Simul.SAVE_FILENAME   = 'custom';    % Name of the saved results file
Param.Simul.SHOW_GRAPHS     = 1;    % Show all graphs (forces, angles, speed, ...)
Param.Simul.SHOW_3DVIEW     = 0;    % Show the 3D view of the whole rotor
...
Param.Simul.CONV_CRIT   = 1e-6; % Convergence criterion value


% ------------ Freestream ------------
Param.Air.ALTITUDE      = 3050;             %!!!!! Altitude, [m]
...
% -------------- Blades --------------
Blades.PROFILE_FILE = 'naca4412.dat';       % Profile coordinates for 3D view of the final blade
Blades.COEFF_FILE   = 'naca4412polar.mat';  % Airfoil polars for various Reynolds

Blades.OMEGArpm = 2000;     %!!!!! Rotational speed, [RPM]
Blades.nELEM    = 100;      % Number of blade elements

Blades.nBLADES          = 6;        % Number of blades on the rotor
Blades.RADIUS           = 0.6;      % Radius (from HUB center to tip), [m]
Blades.ROOT_CHORD       = 0.30;     % Root chord (will only be used if linear/constant taper), [m]
Blades.TIP_CHORD        = 0.10;     % Tip chord, [m]
Blades.ROOT_CUTOUT      = 0.15;     % Root cutout, [m]
Blades.THETA_TIPdeg     = 8;        % Twist angle at tip, [deg]
Blades.COLL_PITCHdeg    = 20;       % Collective pitch, [deg]
...
Elem.CUSTOM_TWIST = []; % Twist of each individual element [deg] (! vector of [Blades.nELEM x 1])

% If Param.Simul.TAPER = 2 (Custom taper)
Elem.CUSTOM_CHORD = []; % Chord of each individual element [deg] (! vector of [Blades.nELEM x 1])
